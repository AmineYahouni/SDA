			...
			insert(i);
   			...
   			analyzer_append(time_analysis, after.tv_nsec - before.tv_nsec);}
                ...
                case 4:
                        ...
                        display(root, 0);
                        break;
                case 9:// Sauvegarde les données de l'expérience.
  			save_values(time_analysis, "../plots/dynamic_array_time_ascenBabre.plot");	
                        exit(1);
                case 5:
...
KeyStatus del(node *ptr, int key) {
        ...
        key_arr = ptr->keys;
        p = ptr->p;
        min = (2*M - 1) / 2;/*Minimum number of keys*/
